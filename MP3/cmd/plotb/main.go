...
func main() {
	if len(os.Args) < 2 {
		log.Fatal("Usage: ./plotb <number of file>")
	}
	...
	log.Printf("%s stopped! Wait for 15s for the system to rebalance.", failureNode)
	...
	if numOfFile > 0 {
		...
		log.Printf("%d file generated! Wait for 15s for the system to rebalance.", fileSize)
		time.Sleep(15 * time.Second)
	}


	// Start the node
	cmd = exec.Command("ssh", failureNode, "-t", "sudo systemctl start MP3_server.service")
	err = cmd.Run()
	if err != nil {
		log.Fatalf("Failed to start the machine: %s", err.Error())
	}
	log.Printf("%s restarted! Start recording for 10s", failureNode)
